...
#@us = ["@4473c422c321463", "@MrnAdrn", "@Nasim51184163", "@DcEnzo"]
...
=end
def bonjour
  @journalist.sample(5).each do |tweet|
  #client.update("#{tweet} Hey ! I am testing a bot for @the_hacking_pro ! #bonjour_monde")
  puts tweet
  end
end
...
def streamfollow(stream)

stream.filter(track:"bonjour_monde")do |tweet|
if tweet.is_a?(Twitter::Tweet)
puts tweet.text
  client.favorite(tweet.id)
  client.follow(tweet.user.screen_name)
